...
void	textured_floor(t_env *e, t_col *col)
{
	while (e->y < MAP_H)
	{
		e->d_wall = e->d_ray;
		...
		e->current_floor_x = e->weight * e->floor_wall_x
			+ (1.0 - e->weight) * e->pos_x;
		e->current_floor_y = e->weight * e->floor_wall_y
			+ (1.0 - e->weight) * e->pos_y;
